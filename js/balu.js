        ...
        if (userid == el.id) {
          for (i = 0; i < el.recipie.length; i++) {
            ...
            if (el.recipie[i].id == recipieid) {
              ...
              const ing = document.getElementById('stepshow');
              const youtube = document.getElementById("youtube");
              
              for(j=0;j<el.recipie[i].ingredients.length;j++){
              const para = document.createElement('p');
              para.innerHTML= "<br>"+el.recipie[i].ingredients[j];
              ing.appendChild(para);
              }
              // console.log(el.recipie[i].ingredients);
